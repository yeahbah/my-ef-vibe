...
Live-database tests against [AdventureWorks](https://github.com/theMickster/AdventureWorks) on your machine. They are **not** run in CI (see `.github/workflows/ci.yml`, which only runs `MyEfVibe.Tests`).

All four providers use the **same** repo: `/home/adiaz/Projects/AdventureWorks`. Databases are provisioned from the SQL Server `AdventureWorks2022` backup via Docker.
