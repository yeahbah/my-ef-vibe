...
| `Could not load file or assembly 'System.Text.Json, Version=9.0.0.0'` (often `SharedFrameworkCatalog`) | efvibe host TFM/runtime mismatch while loading a `net8.0` workspace. Use a current efvibe build, install the **.NET 8 runtime**, run via local tool (`dotnet efvibe` from `.config/dotnet-tools.json`) so the tool matches your app band, and delete any `System.Text.Json.dll` copied into project `bin/`. |
