...
internal sealed class EfSqlCapture : IDisposable
{
    ...
    private EfSqlCapture(object dbContextInstance, DbLogSettings settings,
        DiagnosticSqlCaptureBinding? diagnosticBinding)
    {
        ...
        object dbContextInstance)
    {
        ...
        capture._listenerSubscriptions.Add(
            DiagnosticListener.AllListeners.Subscribe(new DiagnosticListenerObserver(capture, binding,
                capture._settings.Verbose)));
    ...
    {
        return ReferenceEquals(context, _dbContextInstance) || Equals(context, _dbContextInstance);
    }
    ...
    {
        return $"{parameter.ParameterName} = {parameter.Value ?? "NULL"}";
    }
