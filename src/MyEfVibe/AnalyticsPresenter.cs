...
internal static class AnalyticsPresenter
{
    ...
    internal static void WriteCompare(EvaluationMetrics baseline, EvaluationMetrics current)
    {
        ...
        table.AddRow("db ms", baseline.DatabaseMilliseconds?.ToString() ?? "-",
            current.DatabaseMilliseconds?.ToString() ?? "-");
    ...
    {
        return metrics.ExecutedSql.Count == 0
    ...
    {
        return metrics.ResultKind switch
        {
